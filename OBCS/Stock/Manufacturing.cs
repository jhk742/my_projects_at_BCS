...
namespace OBCS.Stock
{
    public partial class Manufacturing : Form
    {
        public Manufacturing()
        {
            ...
            connSalesOrder.sp_MANUFACTURING_SELECT(grid_header, FromDate, ToDate);
        ...
        private void Initialized()
        {
            ...
            connSalesOrder.sp_MANUFACTURING_SELECT(grid_header, dateTimePickerFrom.Value.Date, dateTimePickerTo.Value.Date);
        }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DialogResult dialogResult = MessageBox.Show("Are you sure to continue Manufacturing process?"
                                                            , "Confirming", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dialogResult == DialogResult.Yes)
                {
                    if (!REALTIME_STOCKCHECK()) return;

                    String NewBCSMTKNumber = connSalesOrder.sp_GET_NewBCSMTKNumber();
                    String SaveHeaderResult = connSalesOrder.sp_MANUFACTURING_SAVE_HEADER(NewBCSMTKNumber, AccountingDate, Memo, Company, Site);
                    ...
                    for (int i = 0; i < grid_Basket.RowCount; i++)
                    {
                        ...
                        String SaveDetailResult = connSalesOrder.sp_MANUFACTURING_SAVE(NewBCSMTKNumber, StorageSite, SelectedProductID, LotNumber, Status, Expiration, OrderQtys, Amount, i + 1);

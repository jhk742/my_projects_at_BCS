...
{
    public partial class ManufacturingReceipt : Form
    {
        public ManufacturingReceipt()
        {
            InitializeComponent();
            grid_header_Init();
            ...
            dateTimePickerExpired.Value = DateTime.Now.AddMonths(10);

            DateTime FromDate = dateTimePickerFrom.Value.Date;
            DateTime ToDate = dateTimePickerTo.Value.Date;

            connSalesOrder.sp_MANUFACTURING_SELECT_DONE(grid_header,FromDate,ToDate);
            connSalesOrder.sp_MANUFACTURING_SELECT_TEMP(grid_TempHeader);
        ...
        private void grid_TempHeader_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                String MTKNumber = grid_TempHeader.Rows[e.RowIndex].Cells["Number"].Value.ToString().Trim();
                String Site = grid_TempHeader.Rows[e.RowIndex].Cells["Site"].Value.ToString().Trim();

                connSalesOrder.sp_MANUFACTURING_SELECT_DETAIL(grid_detail, MTKNumber);
                grid_Basket.DataSource = null;
                grid_Basket.Rows.Clear();
                grid_Basket.Refresh();

                txt_MTKNumber.Text = MTKNumber;
                cbo_storageSite.Text = Site;
                dateTimePickerAcc.Value = System.DateTime.Now;
                txt_Memo.Text = "";
                grid_Basket.Columns["Delete"].Visible = true;
                connSalesOrder.sp_MANUFACTURING_SELECT_DETAIL(grid_Basket, "GRID_BASKET_INITIALIZE");
            }
            catch (Exception ex)
            {
        ...
        private void grid_header_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                String MTKNumber = grid_header.Rows[e.RowIndex].Cells["Number"].Value.ToString().Trim();
                String Site = grid_header.Rows[e.RowIndex].Cells["Site"].Value.ToString().Trim();

                txt_MTKNumber.Text = MTKNumber;
                cbo_storageSite.Text = Site;

                connSalesOrder.txt_DisReceipt("ManufacturingH", dateTimePickerAcc, txt_Memo, MTKNumber);
                connSalesOrder.sp_MANUFACTURING_SELECT_DETAIL(grid_detail, MTKNumber);
                connSalesOrder.sp_MANUFACTURINGRECEIPT_SELECT_DETAIL(grid_Basket, MTKNumber);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString());
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                String ProductDesc = txt_ProductDesc.Text.ToString();
                ...
                if (dateTimePickerExpired.Value.Date > DateTime.Now.Date.AddMonths(10))
                {
                    dateTimePickerExpired.Value = DateTime.Now.Date.AddMonths(10);
                }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txt_MTKNumber.Text.Length == 0)
                {
                    MessageBox.Show("Please select MTKNumber first.", "Stop", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                DialogResult dialogResult = MessageBox.Show("Are you sure to continue Manufacturing receipt process?"
                                                            , "Confirming", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dialogResult == DialogResult.Yes)
                {
                    String NewSTKNumber = connSalesOrder.sp_GET_NewSTKNumber();
                    String MTKNumber = txt_MTKNumber.Text.ToString();
                    ...
                    for (int i = 0; i < grid_Basket.RowCount; i++)
                    {
                        ...
                        DateTime ExpirationDate = Convert.ToDateTime(grid_Basket.Rows[i].Cells["Expiration Date"].Value);
                        String StorageSite = grid_Basket.Rows[i].Cells["Site"].Value.ToString();

                        //MessageBox.Show(NewSTKNumber + "\n" + MTKNumber + "\n" + LOTNumber + "\n" + AccountingDate.ToShortDateString() + "\n" + CompanyCode + "\n" + Site + "\n" + ProductID + "\n" +
                        //    Status + "\n" + ExpirationDate.ToShortDateString() + "\n" + Memo + "\n" + Qty.ToString() + "\n" + (i + 1).ToString()); ;

                        String SaveResult = connSalesOrder.sp_MANUFACTURINGRECEIPT_SAVE(NewSTKNumber, MTKNumber, LOTNumber, AccountingDate, CompanyCode, StorageSite, ProductID,
                    ...
                    String SaveHeaderResult = connSalesOrder.sp_MANUFACTURINGRECEIPT_SAVE_HEADER(NewSTKNumber, MTKNumber, AccountingDate, Memo, CompanyCode, Site);
        ...
        private void ManufacturingReceipt_Load(object sender, EventArgs e)
        {

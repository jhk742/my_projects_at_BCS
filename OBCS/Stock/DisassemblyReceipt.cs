...
{
    public partial class DisassemblyReceipt : Form
    {
        public DisassemblyReceipt()
        {
            InitializeComponent();
            grid_header_Init();
            ...
            connSalesOrder.sp_DISASSEMBLY_SELECT_DISASSEMBLED(grid_header,FromDate,ToDate);
            connSalesOrder.sp_DISASSEMBLY_SELECT_TEMP(grid_TempHeader);
        ...
        private void grid_TempHeader_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                String DISNumber = grid_TempHeader.Rows[e.RowIndex].Cells["Number"].Value.ToString().Trim();
        ...
        private void grid_header_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                connSalesOrder.txt_DisReceipt("SMVTH",dateTimePickerAcc,txt_Memo,DISNumber);
                connSalesOrder.sp_DISASSEMBLY_SELECT_DETAIL(grid_detail, DISNumber);
                connSalesOrder.sp_DISASSEMBLYRECEIPT_SELECT_DETAIL(grid_Basket, DISNumber);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString());
        ...
        private void grid_stock_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //    if (grid_stock.Rows[e.RowIndex].Cells["Receipt quantity"].Value == DBNull.Value || Convert.ToDecimal(grid_stock.Rows[e.RowIndex].Cells["Receipt quantity"].Value) < 0)
            //    {
            //        grid_stock.Rows[e.RowIndex].Cells["Receipt quantity"].Value = "0.00";
            //    }
            //    //if (Convert.ToDecimal(grid_stock.Rows[e.RowIndex].Cells["Receipt quantity"].Value) > Convert.ToDecimal(grid_stock.Rows[e.RowIndex].Cells["Available"].Value))
            //    //{
            ...
            //    for (int i = 0; i < grid_stock.RowCount; i++)
            //    {
            //        TotalIssueQty += Convert.ToDecimal(grid_stock.Rows[i].Cells["Receipt quantity"].Value);
            //    }
        ...
        private void DisassemblyReceipt_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    for (int i = 0; i < grid_Basket.RowCount; i++)
                    {
                        ...
                        String SaveResult = connSalesOrder.sp_DISASSEMBLYRECEIPT_SAVE(NewSTKNumber,DISNumber,LOTNumber,AccountingDate,CompanyCode,Site,ProductID, 
                    ...
                    String SaveHeaderResult = connSalesOrder.sp_DISASSEMBLYRECEIPT_SAVE_HEADER(NewSTKNumber,DISNumber,AccountingDate,Memo,CompanyCode,Site);
